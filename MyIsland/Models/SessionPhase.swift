...
enum SessionPhase: Sendable {
    ...
    /// Color for session status dot in minimized notch
    var dotColor: Color {
        switch self {
        case .waitingForApproval:
            return Color(red: 0.3, green: 0.5, blue: 1.0) // Blue - needs approval
        case .processing, .compacting:
            return Color.orange // Orange - working
        case .waitingForInput:
            return Color.green // Green - ready for input
        case .idle:
    ...
    /// Priority for sorting status dots (higher = more important = shown first)
    var dotPriority: Int {
        switch self {
