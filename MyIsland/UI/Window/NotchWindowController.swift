    ...
    init(screen: NSScreen) {
        ...
        viewModel.$status
            ...
            .sink { [weak notchWindow, weak viewModel] status in
                switch status {
                case .opened:
                    let isNotification = viewModel?.openReason == .notification
                    if isNotification {
                        // Notification-triggered: keep mouse pass-through to avoid
                        // interrupting user's current mouse/cursor behavior.
                        // Mouse events will be enabled when user hovers over the panel.
                        notchWindow?.ignoresMouseEvents = true
                    } else {
                        // User-initiated: accept mouse events and activate
        ...
        // When user hovers over the notification panel, enable mouse events
        viewModel.$isHovering
            .receive(on: DispatchQueue.main)
            .sink { [weak notchWindow, weak viewModel] hovering in
                if hovering && viewModel?.status == .opened {
                    notchWindow?.ignoresMouseEvents = false
                }
            }
            .store(in: &cancellables)
