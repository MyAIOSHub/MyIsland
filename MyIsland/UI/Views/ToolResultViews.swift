...
// MARK: - AskUserQuestion Options View (Interactive)

/// Interactive view for pending AskUserQuestion - shows clickable options
struct AskUserQuestionOptionsView: View {
    let questions: [QuestionItem]
    let onSelectOption: ((Int, Int) -> Void)?  // (questionIndex, optionIndex)
            ...
            ForEach(Array(questions.enumerated()), id: \.offset) { qIdx, question in
                VStack(alignment: .leading, spacing: DesignTokens.Spacing.sm) {
                    ...
                    ForEach(Array(question.options.enumerated()), id: \.offset) { oIdx, option in
                        let optionId = "\(qIdx)-\(oIdx)"
                        Button {
                            onSelectOption?(qIdx, oIdx)
                        } label: {
                            HStack(alignment: .top, spacing: DesignTokens.Spacing.sm) {
                                // Number badge — Claude orange
                                Text("\(oIdx + 1)")
                                    .font(.system(size: 10, weight: .bold, design: .monospaced))
                                    .foregroundColor(.black)
                                    .frame(width: 18, height: 18)
                                    .background(Circle().fill(TerminalColors.prompt))
                            ...
                            .background(
                                RoundedRectangle(cornerRadius: DesignTokens.Radius.lg)
                                    .fill(hoveredOption == optionId ? DesignTokens.Surface.pressed : DesignTokens.Surface.elevated)
                            )
                            .overlay(
                                RoundedRectangle(cornerRadius: DesignTokens.Radius.lg)
                                    .strokeBorder(hoveredOption == optionId ? DesignTokens.Border.standard : DesignTokens.Border.subtle, lineWidth: 1)
                            )
                            .contentShape(Rectangle())
                        }
                        .buttonStyle(.plain)
                        .onHover { isHovered in
                            hoveredOption = isHovered ? optionId : nil
                        }
                    }
                }
            }
