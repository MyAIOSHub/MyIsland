...
actor SessionStore {
    ...
    nonisolated static let logger = Logger(subsystem: "com.myisland", category: "Session")
    ...
    private func processHookEvent(_ event: HookEvent) async {
        ...
        if let pid = event.pid {
            ...
            // Detect source app only once (on first event with PID)
            if session.sourceApp == .unknown {
                session.sourceApp = SessionSourceApp.detect(source: event.source, pid: pid, tree: tree)
            }
        }
