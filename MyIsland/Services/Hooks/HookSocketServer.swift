...
struct HookEvent: Codable, Sendable {
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case cwd, event, status, pid, tty, tool, source
...
class HookSocketServer {
    ...
    static let socketPath = "/tmp/myisland.sock"

    private var serverSocket: Int32 = -1
    private var acceptSource: DispatchSourceRead?
    private var eventHandler: HookEventHandler?
    private var permissionFailureHandler: PermissionFailureHandler?
    private let queue = DispatchQueue(label: "com.myisland.socket", qos: .userInitiated)
    ...
    private func cacheToolUseId(event: HookEvent) {
        ...
        if toolUseIdCache.count > maxCacheSize {
            let keysToRemove = Array(toolUseIdCache.keys.prefix(toolUseIdCache.count - maxCacheSize))
            for k in keysToRemove {
                toolUseIdCache.removeValue(forKey: k)
            }
        }
        cacheLock.unlock()
    ...
    /// Safe close — guards against EBADF on already-closed sockets
    private func safeClose(_ fd: Int32) {
        guard fd >= 0 else { return }
        // Check if fd is still valid before closing
