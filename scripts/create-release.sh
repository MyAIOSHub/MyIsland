...
APP_PATH="$EXPORT_PATH/My Island.app"
APP_NAME="MyIsland"
KEYCHAIN_PROFILE="MyIsland"
...
if command -v create-dmg &> /dev/null; then
    ...
    create-dmg \
        ...
        --window-size 600 400 \
        --icon-size 100 \
        --icon "My Island.app" 150 200 \
        --app-drop-link 450 200 \
        --hide-extension "My Island.app" \
        "$DMG_PATH" \
        "$APP_PATH"
else
    echo "Using hdiutil (install create-dmg for prettier DMG: brew install create-dmg)"
    hdiutil create -volname "My Island" \
        -srcfolder "$APP_PATH" \
        -ov -format UDZO \
        "$DMG_PATH"
fi
...
POSSIBLE_PATHS=(
    "$HOME/Library/Developer/Xcode/DerivedData/MyIsland-*/SourcePackages/artifacts/sparkle/Sparkle/bin"
)
...
if ! command -v gh &> /dev/null; then
    ...
    if gh release view "v$VERSION" --repo "$GITHUB_REPO" &>/dev/null; then
        ...
        gh release create "v$VERSION" "$DMG_PATH" \
            --repo "$GITHUB_REPO" \
            --title "My Island v$VERSION" \
            --notes "## My Island v$VERSION

### Installation
1. Download \`$APP_NAME-$VERSION.dmg\`
2. Open the DMG and drag My Island to Applications
3. Launch My Island from Applications

### Auto-updates
After installation, My Island will automatically check for updates."
    fi
